...
import os.common.result.OsResult;
import os.common.util.ServletUtils;
import os.common.util.SingletonLoginUtils;
...
public class UserLoginController extends BaseController {
	...
	@Autowired
	private IUserService webUserService;
	@Autowired
	...
	public Object login(@RequestParam("loginName") String loginName,
		...
		try{
			...
			WebUserLoginLog userLoginLog = new WebUserLoginLog(new Date(), ServletUtils.getIpAddr(),
					SingletonLoginUtils.getUserId(), ServletUtils.getUserOperatingSystem(),
					ServletUtils.getUserBrowser());
			Integer count = webUserService.updateLogById(SingletonLoginUtils.getUserId(), userLoginLog);
	...
	@ResponseBody
	public Object register(@ModelAttribute("user") WebUser user, @RequestParam("registerCode") String registerCode) {
		...
		try {
            webUserService.insertUser(user);
	...
	public Object emailActive(@RequestParam("email") String email, @RequestParam("captcha") String captcha,
		...
		// 激活该账号
		Integer count = webUserService.updateEmailActive(email);
		// 更新链接已失效
	...
	public Object perfectUser(@RequestParam("email") String email, @RequestParam("telephone") String telephone,
		...
		try {
            webUserService.updatePerfectUser(email, telephone, realName);
	...
	public Object postForgetPassword(@RequestParam("email") String email,
		...
		if (webUserService.getByEmail(email) == null) {
	...
	public Object resetPassword(@RequestParam("email") String email, @RequestParam("captcha") String captcha,
		...
		// 更新账户密码
        webUserService.updatePasswordByEmail(loginPassword, email);
		return new OsResult(CommonReturnCode.SUCCESS, email);
	}	
